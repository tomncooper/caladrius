...
def get_comp_links_by_grouping(
    ...
    component_connections: List[Dict[str, str]] = (
        ...
        .has("grouping", grouping)
        .as_("stream")
        .inV()
        .as_("destination")
        .select("source", "stream", "destination")
        .by("component")
        .by("stream")
        .by("component")
        .dedup()
...
def set_shuffle_routing_probs(
    ...
    LOG.info(
        "Calculating routing probabilities for shuffle grouped logical "
        "connections in the graph of topology %s reference %s",
        topology_id,
        topology_ref,
    )

    topology_traversal: GraphTraversalSource = graph_client.topology_subgraph(
    ...
    for comp_conn in get_comp_links_by_grouping(topology_traversal, "SHUFFLE"):
        ...
        LOG.debug(
            "Calculating routing probabilities for logical connections "
            "between instances of %s and %s on the %s stream",
            comp_conn["source"],
            comp_conn["destination"],
            comp_conn["stream"],
        )
        ...
        (
            topology_traversal.V()
            .has("component", comp_conn["source"])
            .outE("logically_connected")
            .has("stream", comp_conn["stream"])
...
def set_fields_routing_probs(
    ...
    LOG.info(
        ...
        "reference %s using metrics data from %s to %s",
        topology_id,
        topology_ref,
        start.isoformat(),
        end.isoformat(),
    ...
    i_to_i_rps.set_index(["source_task", "stream", "destination_task"], inplace=True)
    ...
    LOG.debug(
        "Processing %d fields grouped connections for topology %s " "reference %s",
        len(fields_connections),
        topology_id,
        topology_ref,
    )
    ...
    for connection in fields_connections:
        ...
        LOG.debug(
            "Processing connection from instance %d to %d on stream %s",
            connection["source_task"],
        ...
        (
            ...
            .property("routing_probability", routing_prob)
            .next()
        )
