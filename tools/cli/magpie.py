...
LOG: logging.Logger = logging.getLogger("Magpie_CLI")


class Magpie(object):
    """ This is a helper context class for all the Magpie DSPS sub-commands """
...
@click.argument("url", required=True, envvar="MAGPIE")
@click.option(
    "--debug/--no-debug",
    required=False,
    default=False,
...
def cli(ctx, url, debug):
    """ Command line interface for the Magpie DSPS modelling server. This program
    ...
    or read from the MAGPIE environment variable.
    """
    ...
    if debug:
        ...
        formatter = logging.Formatter(
            (
                "{levelname} | {name} | function: {funcName} "
                "| line: {lineno} | {message}"
            ),
            style="{",
        )
    ...
    if "http://" not in url:
        LOG.debug(
            "Warning: http:// was not included in the supplied URL. This will be "
            "added for all calls to the server"
        )
        url = "http://" + url

    ctx.obj = Magpie(url, debug)

    LOG.debug(f"Connecting to Magpie server at: {url}")
