...
within the magpie graph database. """
...
from magpie.common.timestamp import get_window_dt_from_now

LOG: logging.Logger = logging.getLogger("magpie.tools.heron.graph_builder")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Builds a physical graph representation of the specified" "heron topology "
        )
    )
    parser.add_argument(
        "-cfg",
    ...
    parser.add_argument(
        "-t", "--topology", required=True, help="The topology identification string"
    )
    parser.add_argument(
        "-z", "--zone", required=True, help="The zone the topology is run within"
    )
    parser.add_argument(
        "-e",
        "--environment",
        required=True,
        help=("The environment the topology is run within. " "eg TEST, PROD etc."),
    )
    parser.add_argument(
        "-r",
        "--reference",
        required=True,
        help=(
            "The topology reference to be applied to all "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Flag indicating if the physical graph should "
            "be populated with metrics. This requires some "
            "custom magpie metric classes to be included "
            "in the topology."
        ),
    )
    parser.add_argument(
        ...
        required=False,
        help=(
            "The time in second from now backwards over "
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help=("Optional flag indicating if debug logging " "output should be shown"),
    )
    parser.add_argument(
        "-q",
        "--quiet",
        required=False,
        action="store_true",
        help=("Optional flag indicating if log output should " "be suppressed."),
    )
    return parser
...
if __name__ == "__main__":
    ...
    if ARGS.populate and not ARGS.duration:
        MSG: str = (
            "Populate flag was supplied but duration argument "
    ...
    if ARGS.populate and ARGS.duration:
        ...
        try:
            builder.populate_physical_graph(
                GRAPH_CLIENT, METRICS_CLIENT, ARGS.topology, ARGS.reference, START, END
            )
        except KeyError as kerr:
            err_msg: str = (
                "Magpie metrics not present in metrics "
    ...
    LOG.info(
        "Graph building completed in %d seconds",
