...
def build_component_models(
    ...
    LOG.info("Creating traffic models for spout components of topology %s", topology_id)

    if start and end and not spout_emits:
        spout_emits = get_spout_emissions(
            metric_client, tracker_url, topology_id, cluster, environ, start, end
        )
    elif not start and not end and not spout_emits:
        err: str = (
            "Either start and end datetime instances or the spout "
    ...
    for (spout_comp, stream), data in spout_comp_emits.groupby(["component", "stream"]):

        LOG.info("Creating traffic model for spout %s stream %s", spout_comp, stream)

        df: pd.DataFrame = (
            data[["timestamp", "emit_count"]].rename(
                index=str, columns={"timestamp": "ds", "emit_count": "y"}
            )
        )
...
def predict_per_component(
    ...
    models: DefaultDict[str, Dict[str, Prophet]] = build_component_models(
        metric_client, tracker_url, topology_id, cluster, environ, start, end
    )
...
def build_instance_models(
    ...
    if start and end and spout_emits is None:
        ...
        err: str = (
            "Both start and end datetimes should be provided if spout "
            "emits DataFrame is not"
        )
        LOG.error(err)
        raise RuntimeError(err)
    elif spout_emits is None and not start and not end:
        err = "Either start and end datetimes or the spout emits should be " "provided"
        LOG.error(err)
        raise RuntimeError(err)
    else:
    ...
    spout_groups: pd.core.groupby.DataFrameGroupBy = (
        spout_emits[["component", "task", "stream", "timestamp", "emit_count"]].groupby(
...
def run_per_instance_models(models: INSTANCE_MODELS, future_mins: int) -> pd.DataFrame:
...
class ProphetTrafficModel(HeronTrafficModel):
    ...
    description: str = (
        "This model uses the Facebook Prophet time series "
    ...
    def __init__(
        ...
        if "prophet.model.default.source.hours" in config:
            ...
            self.default_source_hours: float = config[
                "prophet.model.default.source.hours"
            ]
        ...
        if "prophet.model.default.future.mins" in config:
            ...
            self.default_future_mins: int = config["prophet.model.default.future.mins"]

        else:
            self.default_future_mins = 30
            LOG.warning(
                "Default future minutes parameter was not supplied "
    ...
    def predict_traffic(
        ...
        **kwargs: Union[str, int, float],
        ...
        source_end: dt.datetime = dt.datetime.utcnow().replace(tzinfo=dt.timezone.utc)
        source_start: dt.datetime = (source_end - dt.timedelta(hours=source_hours))

        if "future_mins" not in kwargs:
            LOG.warning(
                "future_mins parameter (indicating how many minutes "
        ...
        LOG.info(
            "Predicting traffic over the next %d minutes for topology %s "
        ...
        traffic_by_component: pd.core.groupby.DataFrameGroupBy = component_traffic.groupby(
            ["component", "stream"]
        )

        components: DefaultDict[str, DefaultDict[str, Dict[str, float]]] = defaultdict(
        ...
        for (spout_component, stream), data in traffic_by_component:
            ...
            for quantile in self.quantiles:
                components[f"{quantile}-quantile"][spout_component][stream] = (
                    float(data.yhat.quantile(quantile / 100)) / time_period_sec
                )
        ...
        traffic_by_task: pd.core.groupby.DataFrameGroupBy = instance_traffic.groupby(
            ["task", "stream"]
        )

        instances: DefaultDict[str, DefaultDict[str, Dict[str, float]]] = defaultdict(
            lambda: defaultdict(dict)
        )
